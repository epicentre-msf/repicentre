...
Cette session accompagne l'étude de cas _Réponse d'urgence contre la rougeole dans la région du Katanga (RDC)_ du module FETCH Surveillance. Ça n'a probablement pas beaucoup de sens d'essayer de la suivre sans les documents et les discussions de l'étude.
...
::: {.callout-tip}
N'hésitez pas à scaner vos notes, vos scripts ou les tutoriels des sessions précédentes pour vous rafraîchir sur le fonctionnement de certaines fonctions quand vous en éprouvez le besoin.

Par ailleurs, les [encadrés contenant les instructions](../sessions_core/01_introduction.qmd#Exercise Format) sont les mêmes que dans les autres modules.
:::


## Mise en place

Comme cette session fait partie d'un module spécifique, vous allez créer un nouveau projet RStudio. Jetez un coup d’œil [à la session principale](../sessions_core/02_import_data.qmd) si vous ne vous rappelez plus comment faire.

### Créer le projet

:::  {.setup}
1. Créez un dossier `surveillance_case_study` sur votre ordinateur, associé avec le module Surveillance du FETCH. Ajoutez les sous dossiers suivants à l'intérieur :
...
2. Créez un [projet RStudio](../sessions_core/02_import_data.qmd#rstudio-projects) à la racine du dossier `surveillance_case_study`.

3. Si vous n'avez pas encore les données, téléchargez-les.
...
downloadthis::download_link(
  ...
  button_label = "Télécharger les données brutes",
  has_icon = TRUE,
  icon = "fa fa-save",
...
:::  {.write}
1. Ajoutez une section pour l'import des données à votre script.

2. Importez le jeu de données surveillance et stockez le dans un objet `df_surv_brut`. Ensuite, importez le jeu de données laboratoire et stockez le dans un objet `df_labo_brut`. 

3. Vérifiez que l'importation s'est bien passée pour les deux data frames (vous avez plusieurs outils à votre disposition : Viewer, dimensions de l'objet, haut et bas du data frame...).
:::



## Nettoyage des données (Question 2)
...
Maintenant que nous avons une meilleure idée de l'état des données, nettoyons-les. Nous allons écrire un *pipeline de nettoyage* (ou *chaîne de commandes*) comme dans les modules R précédents (voir votre code à la fin du [module de nettoyage](../sessions_core/04_data_verbs_conditional.qmd)).
...
Nous allons maintenant voir deux petites fonctions très utiles pour le nettoyage du texte. La première est la fonction `str_squish()` du paquet `{stringr}` ([la page d'aide](https://stringr.tidyverse.org/reference/str_trim.html)), qui supprime les espaces au début ou à la fin des chaînes de caractères, et  les espaces surnuméraires où qu'ils soient : 
...
L'autre fonction, `str_replace` (également du [paquet `{stringr}`](https://stringr.tidyverse.org/reference/str_replace.html?q=str_replace#null)) remplace un bout de texte dans une chaîne de caractères par un autre bout de texte, sans surprise. L'argument `pattern` accepte le texte à remplacer, et l'argument `replacement` le texte à utiliser comme remplacement.
...
1. Démarrez un nouveau *pipeline* de nettoyage pour les données labo. Choisissez une colonne de texte et passez là en minuscules, puis supprimez les espaces surnuméraires. Enfin, remplacez les espaces et les `-` par `_`.

2. Recodez au moins une des colonnes `igm_rougeole` ou `igm_rubeole` pour que les catégories soient `negatif`, `positif` et `indeterminé`.

3. Stockez la version nettoyée dans un data frame `df_labo`
...
### Données de surveillance complétées
...
3. Stockez le résultat dans un data frame appelé `df_surv_sem`, qui devrait ressembler à :
...
4. Exportez ce data frame dans un fichier `.rds` appelé `data_ids_2022-w20_weeks_clean` dans le sous dossier `data/clean` de votre projet.
:::
...
Vous pouvez aussi lire les explications sur la fonction `complete()` à la fin du tutoriel et aller explorer la [page d'aide](https://tidyr.tidyverse.org/reference/complete.html) de la fonction.
...
Nous allons continuer la préparation des data frames pour l'analyse.

#### Mise en place

::: {.setup}
1. Si vous n'avez pas eu le temps de nettoyer la zone de santé et la province dans les deux data frames, et les deux colonnes IgM dans le jeu de données labo, vous pouvez importer les jeux de données nettoyés :
...
downloadthis::download_link(
  ...
  button_label = "Télécharger données propres",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
)
...
2. Créez un script `analyse_surveillance.R` dans le sous dossier `R`. Ajoutez les métadonnées, et une section pour importer les paquets `{here}`, `{rio}`, `{tidyverse}`, `{lubridate}` et `{zoo}`.

3. Ajoutez une section  d'import des données propres et importez les fichiers `.rds` dans R en utilisant la fonction `import()` comme d'habitude (soit vos données nettoyées, soit celles que vous venez de télécharger). Assignez les jeux de données nettoyées aux data frames suivants :
...
Notre premier indicateur regarde si *une zone de santé a 20 cas suspects ou plus dans une semaine*. Cet indicateur est dichotomique et ne prend en compte que les données d'une zone de santé pour une semaine donnée.
...
Notre second indicateur regarde si une zone de santé compte plus de *35 cas suspects cumulés en trois semaines*. C'est un peu plus compliqué à calculer que l'indicateur hebdomadaire : pour chaque zone de santé, il faut calculer la somme des cas par fenêtres de trois semaines, mais les fenêtres ne sont pas fixes, elles *glissent* dans le temps. Nous entrons ici dans le domaine des moyennes/sommes/etc. *mobiles* ou *glissantes*...

##### Somme cumulée

Nous allons utiliser la fonction `rollapply()` du paquet `{zoo}` pour calculer la somme cumulée car elle est polyvalente et puissante. Comme son nom l'indique, la fonction `rollapply()` *applique* une fonction de manière glissante (*roll* peut être traduit ici en "rouler") à un vecteur ou à une colonne d'un data frame. En d'autres termes, cette fonction récapitule pour une colonne donnée les informations de plusieurs *lignes consécutives* d'un data frame avec une fonction de notre choix (somme, moyenne etc.).

Comme nous sommes contraint par le temps, nous allons vous fournir ici le code pour calculer la somme cumulée, et nous vous donnons plus de détails sur la fonction dans la section [Aller plus loin](#sec-going-further) que vous pourrez lire quand vous aurez le temps.
...
Ok, mais nous voudrions utiliser cette fonction dans un data frame qui contient plusieurs zones de santé, et faire la somme cumulée *par zone de santé*. Ce n'est pas si compliqué : nous allons trier notre jeu de données par zone de santé et semaine, puis utiliser l'argument `.by` dans `mutate()` pour effectuer les actions *par zone de santé*.

::: {.callout-note}
Rappelez-vous, nous avons déjà vu le `.by` par le passé: nous l'avons utilisé au sein de la fonction `summarize()` lors de la session sur les [tableaux agrégés](../sessions_core/05_summary_table.qmd#sec-stratify) pour faire des résumés *par groupe*.


C'est la même idée aujourd'hui, sauf qu'au lieu d'utiliser une fonction qui ne renvoie qu'une seule valeur par groupe (`summarize()`) nous allons utiliser une fonction qui renvoie une valeur par **ligne** (`mutate()`), mais prend en compte les informations du groupe.

Pour rappel de comment `summarize()` + `.by` fonctionne, voici comment calculer le nombre total de cas suspects et décès par province : 
...
::: {.write}
1. Ajoutez une étape à votre pipeline pour trier les données par province, zone de santé et semaine (dans cet ordre) avec la fonction `arrange()`, qui est une fonction de tri du paquet `{dplyr}` : 
...
2. Ajoutez ensuite le code pour calculer la somme cumulée : 
...
::: {.callout-important}
La fonction `rollapply()` considère l'ordre du data frame pour décider où placer sa fenêtre, elle n'a pas de *compréhension* du temps : elle fait juste la somme des valeurs précédentes ou suivantes. 

Par exemple, si l'on veut calculer la somme cumulée en semaine 15, la fonction somme les deux valeurs venant avant dans le data frame à la valeur de la ligne concernée. On veut donc que les deux lignes précédentes contiennent les valeurs des semaines 13 et 14 ! Ça implique que :

1. Le data frame est **ordonné** (ou *trié*) ! Si les deux lignes précédentes ne contiennent pas les semaines 13 et 14 de cette zone de santé, la somme cumulée est fausse.
...
Maintenant que la somme cumulée est calculée, on peut calculer l'indicateur dichotomique qui résume les données cumulées pour chaque semaine.
...
Calculons finalement l'indicateur combiné qui résume les deux indicateurs précédents.

::: {.write}
1. Dans le même `mutate()` que précédement, ajoutez une colonne `alerte`, qui est `1` si l'indicateur `cas20` **OU** l'indicateur `cas_cumu35`  est `1` et `0` sinon. Pour le test logique vous devrez utiliser l'opérateur `|` qui représente le **OU** logique. Cet opérateur renvoi `TRUE` si au moins une des conditions est remplie (et donc aussi si le deux sont remplies).

2. Assignez le résultat à un data frame appelé `data_alerte`.
...
Nous allons à présent tracer la courbe épi pour les zones en alerte à la semaine 20. Nous pouvons réutiliser le code vu lors de la session sur les [courbes épidémiques](../sessions_core/06_epicurves.qmd) : nous utiliserons le paquet `ggplot()` et la fonction `geom_col()` pour créer un diagramme qui montre la distribution des cas par semaine.

Petite nouveauté : par le passé nous avions une liste linaire où une ligne correspond à un patient, donc un cas. Aujourd'hui nous avons des données déjà agrégées par semaine et zone de santé : nul besoin de *compter* le nombre de cas nous même.
...
Nouvelle fonction utile, la fonction `facet_wrap()` permet de créer plusieurs graphiques d'un seul coup rassemblés en une seule figure. 
...
::: {.write}
Tracez la courbe épidémique pour les deux zones en alerte en ajustant votre code et en utilisant `facet_wrap()`.
:::
...
::: {.callout-tip}
Cette partie utilise les fonctions d'aggrégation vues quand nous avons appris à faire des [tableaux résumés](../sessions_core/05_summary_table.qmd). N'hésitez pas à vous rafraîchir la mémoire si besoin.
:::
...
::: {.write}
1. Ajoutez-lui une colonne `cas_moins_5ans` qui contient le nombre total de cas rapportés ayant moins de cinq ans.

2. Dérivez, pour chaque zone en alerte, les indicateurs suivants, organisés en un seul tableau : 
...
::: {.callout-tip}
Si vous avez du mal avec cette question, rafraîchissez-vous sur les [résumés conditionnels](../sessions_core/05_summary_table.qmd#sec-summary-cond).
:::
...
Nous allons ici donner quelques éléments pour comprendre un peut les arguments de la fonction `complete()`.

Créons un mini exemple où la zone de santé de Kitenge n'a pas de ligne pour la semaine 2 :
...
C'est problématique si l'on veut calculer une somme cumulée.

Nous pouvons utiliser le code suivant pour compléter toutes les zones de santé pour qu'elles aient toutes les semaines possible, ici les semaines une à trois : 
...
exemple_df |> 
  complete(
    nesting(province, zone_sante), # Par province et zone, les explications arrivent juste après !
...
Comme la base de données va être mise à jour chaque semaine, il serait pratique de choisir *automatiquement* la plage des semaines qui doivent être présentes dans les données.
Pour ça, il nous suffit de remplacer les valeurs que nous avons codé *en dur* par la plus petite semaine existante dans les données et la plus grande :
...
::: {.callout-important}
Gardez à l'esprit que les deux premières valeurs du vecteur (ou de la colonne) contiennent donc des sommes *partielles*. En conséquence, une *absence d'alerte* dans les deux premières semaines ne veut pas forcement dire qu'il n'y a pas d'alerte.
:::
...
Enfin, quelques mots sur l'argument `align`. Il définit la position de la fenêtre glissante par rapport à la valeur en train d'être calculée.
Par défaut la fenêtre est centrée autour de la valeur à calculer : la valeur calculée *i* est la somme des valeurs *i-1* (la valeur de la ligne précédente) et la valeur *i+1* (la valeur de la ligne suivante).
...
# Alignement à gauche : la valeur est la somme des valeurs suivantes dans le data frame
...
# Alignement à droite : somme des valeurs précédentes dans le data frame
rollapply(data  = c(5, 10, 1, 2, 5, 10),
          width = 3, 
          FUN   = sum,
          align = "right",
...
Dans notre cas, nous voulons que la valeur pour une semaine donnée reflète cette semaine et les deux semaines précédentes, donc nous utilisons l'argument `align = "right"`, pour calculer *dans le passé* (si le data frame est ordonné dans l'ordre chronologique pour chaque zone de santé !).
