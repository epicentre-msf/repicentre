...
Cette session accompagne l'étude de cas _Réponse d'urgence contre la rougeole dans la région du Katanga (RDC)_ du module FETCH Surveillance. Ça n'a probablement pas beaucoup de sens d'essayer de le suivre sans les documents et les discussions de l'étude.
...
Vous pouvez aussi lire les explications sur la fonction `complete()` à la fin du tutoriel et allez explorer la [page d'aide](https://tidyr.tidyverse.org/reference/complete.html) de la fonction.
...
Nous allons continuer la préparation de data frames pour l'analyse.
...
::: {.callout-important}
La fonction `rollapply()` considère l'ordre du data frame pour décider où placer sa fenêtre, elle n'a pas de *compréhension* du temps : elle fait juste la somme des valeurs précédentes ou suivantes. En d'autres termes si on veut calculer la somme cumulée en semaine 15, la fonction somme les deux valeurs venant avant dans le data frame à la valeur de la ligne concernée. On veut donc que les deux lignes précédentes contiennent les valeurs des semaines 13 et 14 ! Ça implique que :
...
2. Le data frame est **complet** : il y a des valeurs pour les semaines 14 et 15. Si la somme en semaine 15 en en fait la somme des semaines 12, 13 et 15 parce que la semaine 14 est manquante, le calcul est faux.
:::
...
::: {.callout-tip}
Cette partie utilise les fonctions d'aggrégation vues quand nous avons appris à faire des [tableaux résumés](../sessions_core/05_summary_table.qmd). N'hésitez pas à vous rafraîchir si besoin.
:::
