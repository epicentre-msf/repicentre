...
2. Recode at least one of  `igm_measles` or `igm_rubella` columns so that the categories are `negatif`, `positif` and `indetermine`.
...
::: {.callout-important}
The `rollapply()` function does not understand the *meaning* of time: it only considers the *order* of the data frame to move its window, considering the values of rows before or after. For example, to caclulate the cumulative sum in week 15, the function adds the values of the two previous rows of the dataframe to the one of week 15. We thus want the previous rows to contain the values of week 13 and 14 of that health zone, which implies the following:
...
2. The data frame is **complete**: there are values for all weeks. If week 14 were missing, the cumulative sum would sum the values of week 14, 13 and 12, which is wrong.
:::

##### Calculate the indicator

Now that twe computed the cumulative sum we can calculate the binary indicator for each week and health zone, which is easier. 
...
In our case we want the value for a given week to reflect this week and the week past, so we align the window right, to calculate backward in time (if the data frame is sorted in chronological order by health zone!).
