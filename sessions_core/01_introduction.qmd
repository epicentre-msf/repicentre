...
- You use a rare command or package that your colleague may not know or may find counter intuitive. You can use a comment to explain the rationale behind that decision.
...
Another syntax to extract the various columns of a data frame:
...
Often, a function will have a combination of both *required* and *optional* arguments. The first argument of a function is almost always required and is typically a dataset. As an obligatory and rather obvious argument, most people omit its name when calling a function; i.e. people will write `mean(cases)` instead of `mean(x = cases)`. Optional arguments on the other hand are usually added using their name, i.e.: `mean(x = cases, na.rm = TRUE)`.
