...
*R* is a [functional programming language]{.hoverinfo toggle='tooltip' title='As opposed to object oriented languages which are more adept at building complex programs (such as operating systems).'} that can be used to clean and manipulate data, run analyses (especially statistical ones), visualize results, and much more. 

*RStudio* is a piece of software that provides a user-friendly interface for R (also called an IDE, for Integrated Development Environments). While using a graphical interface isn’t technically required, it is strongly recommended for beginners.
