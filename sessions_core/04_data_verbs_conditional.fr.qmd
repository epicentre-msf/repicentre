...
Dans la session précédente vous avez appris les bases du traitement de données en R avec les fonctions du `{tidyverse}`, en particulier comment sélectionner et modifier les colonnes d'un data frame. Dans cette session nous allons aller plus loin et apprendre à :
...
Mise en place
----------------------------------------------------------------------------------
**Prérequis** : les bases de la manipulation de données avec `{dplyr}`, et en particulier la fonction `mutate()`. Allez vous [rafraîchir si besoin](03_data_verbs.qmd). 

Nous allons poursuivre l'étude de cas sur la rougeole à Mandoul ; **si vous n'avez pas encore suivi les autres sessions du cours, [téléchargez le dossier du cours](https://github.com/epicentre-msf/repicentre/raw/main/data/scenarios/TCH_measles/mandoul_rougeole.zip) avant de continuer**.


::: {.setup}
Nous utiliserons la liste linéaire contenant les données **brutes**.
<br><br>
...
Après avoir appris à sélectionner les *colonnes* d'un data frame nous allons voir comment sélectionner les *lignes*. Sélectionner des observations qui satisfont à [certains critères]{.hovertip bs-toggle='tooltip' bs-title="Par exemple, ne considérer que les patients hospitalisés ou venant d'une région particulière."} est une tâche particulièrement courante du travail d'épidémiologiste. Le paquet `{dplyr}` possède évidement une fonction pour ça : la fonction `filter()`. 
  
Avant de pouvoir l'utiliser nous allons néanmoins devoir apprendre à écrire des **conditions logiques**, qui sont également un prérequis pour recoder des variables. Les conditions logiques sont des questions (ou *tests*) auxquelles R va répondre par `TRUE` [vrai], `FALSE` [faux] ou `NA`.

La syntaxe de `filter()` est assez simple : la fonction *conserve* les lignes où la `condition_logique` est vraie :
...
Dans ce cas, la condition logique teste si quelque chose est égale à autre chose. Par exemple, si telle variable est égale à telle valeur (*est ce que patient a été hospitalisé ?*). 
...
Ici, la fonction `filter()` parcourt chaque ligne de notre data frame et teste si la valeur d'`hospitalisation` dans cette ligne est égale à `"oui"`. Seules les lignes où la réponse à la question est `TRUE` sont renvoyées. 
...
Parfois, nous préférons tester l'inégalité plutôt que l'égalité, en utilisant l'opérateur `!=`. 

Par exemple, pour examiner les patients qui ne se sont **pas** rétablis, qu'ils soient décédés ou sortis contre avis médical, nous écrivons : 
...
Dans le cas des variables numériques, on sera souvent intéressé par savoir si une valeur est supérieure ou inférieure à un seuil. Par exemple, quels sont les patients de moins de 5 ans. Ici, nous utiliserons les opérateurs `<` et `>` pour évaluer si une variable est strictement **inférieure à** ou **supérieure à** une valeur donnée, respectivement. 
...
:::{.write}
Affichez un data frame ne contenant que les patients souffrant de [malnutrition aiguë sévère]{.hovertip bs-toggle='tooltip' bs-title="C'est-à-dire les patients dont le PB est inférieur à 115"}. Le début de la colonne concernée est :
...
Les conditions logiques sont également très utiles lorsque nous voulons [recoder des variables]{.hovertip bs-toggle='tooltip' bs-title="Le recodage est le processus qui consiste soit à standardiser les valeurs d'une variable (par exemple, s'assurer que 'f' et 'femme' sont tous deux codés par 'Femme'), soit à créer des catégories à partir d'une variable continue (par exemple, des groupes d'âge)."}. Dans cette leçon, nous les utiliserons au sein de la fonction `case_when()` (également du paquet `{dplyr}`) qui permet de recoder des variables. Cette fonction est un poil plus complexe que ce que l'on a vu jusqu'à présent, mais très puissante (et donc *très utile*). Nous allons décomposer sa syntaxe pas à pas. 
...
Ainsi, pour recoder les patients avec un PB inférieur à 115mm comme `"MAS"`, nous écrivons la commande suivante dans notre `case_when()` :
...
La dernière ligne de notre pseudo code contient l'argument `.default` et fourni la valeur à utiliser lorsqu’aucune des conditions n'est remplie. Dans notre exemple, ça pourrait être `"Normal"`. 
...
- Est-ce que `pb < 115` pour ce patient ? **Si oui**, attribuer la valeur `"MAS"`
...
df_brut |>
  mutate(groupe_age = case_when(age < 60 ~ "< 5 Ans",
                               age <= 180 ~ "5 - 15 Ans",
                               age > 180 ~ "> 15 Ans",
                               .default = "Inconnu")) |>
  select(age, groupe_age) |>
  head(10)
```
:::
...
:::{.write}
Utilisez `case_when()` et l'opérateur `%in%` pour créer une nouvelle colonne `vacc_status_simple` qui a la valeur :

- `"Oui"` si le statut vaccinal est [confirmé par carte **OU** oralement]{.hovertip bs-toggle='tooltip' bs-title="C'est à dire a un statut vaccinal 'Oui - carte' ou 'Oui - oral'."}
...
df_brut |>
  ...
  select(statut_vaccinal, statut_vaccinal_simple) |>
  head()
```
:::
...
Maintenant que vous savez utiliser les conditions logiques pour recoder des colonnes avec `case_when()`, nous pouvons reprendre le pipeline de nettoyage que nous avions commencé dans la session [précédente](03_data_verbs.qmd).
