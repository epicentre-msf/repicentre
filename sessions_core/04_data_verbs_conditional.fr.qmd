...
1. **Filtrer** : créez des data frames qui ne contiennent que les lignes suivantes :
...
2. **Nettoyage** : ajoutez des étapes à votre chaîne / pipeline de nettoyage
...
3. **Simplification du code** : le code suivant s'exécute sans erreurs, mais pourrait être raccourci car l'auteur n'a pas pris en compte certaines des propriétés de `case_when()`. Simplifiez la commande. N'oubliez pas de vérifier à la fin que vous obtenez le même résultat !
...
4. **Conditions** : les tests logiques ne se restreignent pas à `filter()` et `case_when()`. On peut aussi les utiliser avec des vecteurs. Si nous créons les deux vecteurs suivants : 
...
5. Ecrivez une commande pour tester si la structure de santé "Moissala Ouest" est présente dans la liste linéaire.
