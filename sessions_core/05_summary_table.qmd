...
df_linelist <- rio::import(file = here::here('data', 'scenarios', 'TCH_measles',
                                             'clean', 'moissala_linelist_clean_EN.rds'))
```
...
**Dependencies.** This session assumes that you know how to use RStudio that you are able to import data and that you know the basic data handling verbs that we have seen in the core sessions so far. If you need a refresher on any of these topics, we encourage you to review the core sessions in the learning pathway. 
...
:::{.setup}
This session will use the **clean** version of the Mandoul measles dataset. 
<br><br>
...
### Counting with a Condition {#sec-summary-cond}
...
1.  Make an *age-stratified* summary table that summarizes:
...
### Challenge Exercises
1. Create a table stratified by sub-prefecture that **counts** the number of patients that died among the ones that are *\< 6 months old*. **Hint.** You want to count rows (so use `sum()`) that fill a specific condition for outcome (`outcome == "dead"`), but *only* when `age_group == "< 6 months"`.
