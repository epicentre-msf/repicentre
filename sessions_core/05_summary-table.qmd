...
:::{.setup}
Open your RStudio Project and create a new script in the R subfolder called `tables.R` with appropriate metadata and a "Packages" section that imports: `{rio}`, `{here}` and `{tidyverse}`. Add an "Import Data" section that loads the *cleaned* version of the measles linelist into R.
:::
...
Sometimes we want to produce a statistic *other than counting* given a condition, let's say the *mean age of all hospitalized patients*. Here the syntax is a bit different: 
...
- Indicated which column we want to use when calculating the summary statistic (here a mean)
- Indexed that column using a logical test

For example to compute the mean of the `age` variable but *only* for hospitalized patients (ie in rows where `hospitalisation == "yes"`) we would write:
...
Make a summary table that compute the following statistics by *age groups*: 
