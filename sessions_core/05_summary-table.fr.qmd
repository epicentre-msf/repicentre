...
- Réviser comment sous-trier les lignes en utilisant `filter()` et créer/modifier des variables avec `mutate()`
...
## Comptage de plusieurs colonnes (tableaux de contingence)
...
:::{.write}
Créez un nouveau tableau récapitulatif en comptant le nombre de patients stratifiés par `sub_prefecture` et `hospitalisation`. Que se passe-t-il si vous modifiez l'ordre des arguments donnés à `count()` ?
...
- Quelles sont toutes les valeurs possibles de la variable `outcome` ?
- Combien de patients âgés de 1 à 4 ans se sont rétablis ?
:::

## Filtrer les valeurs `NA`

En examinant les catégories de `outcome`, vous devriez avoir remarqué que certaines patientes ont des valeurs manquantes (`NA`):
...
La proportion de cas décédés est également appelée [taux de létalité]{.hovertip bs-toggle='tooltip' bs-title='Le taux de léthalité est un ratio (ou un risque) mais pas un taux !'}. Pour calculer précisément le CFR, nous devons nous assurer que le dénominateur ne comprend que les patients dont nous sommes sûrs du résultat (c'est-à-dire que nous devons supprimer tous les cas avec "NA" ou "contre avis médical"). 

Rappelons que nous pouvons le faire en utilisant `filter()`. Pour filtrer les valeurs manquantes (`NA`) dans une variable, nous pouvons utiliser la petite fonction `is.na(outcome)`. L'ajout d'un `!` devant fera l'inverse : *supprimer les valeurs manquantes de `outcome`* :

```{r}
df_linelist |>
  filter(outcome != "left against medical advice", 
         !is.na(outcome)) |>
  count(outcome)
```
...
:::{.write}
À l'aide de votre filtre, mettez à jour votre code pour résumer le nombre observé de patients qui ont survécu et sont décédés ainsi que le taux de léthalité (proportion de décès). Stockez ce nouveau dataframe dans un objet, `cfr_df`. 
:::
...
Maintenant que nous avons produit quelques tableaux de fréquence et de contingence simples, nous pouvons augmenter la complexité. Une tâche courante en épidémiologie consiste à examiner les statistiques récapitulatives dans des sous-ensembles de données. 
...
C'est exactement pour cela que la fonction `summarize()` a été créée ! Elle nous permet de calculer des statistiques récapitulatives sur un ensemble de données, et la syntaxe est similaire à celle de `mutate()` : 
...
df_linelist |>
  summarize(.by = sex, # Faire le résumé (ici, la moyenne) par sexe
            mean_age = mean(age))
```
...
Maintenant que nous pouvons utiliser `summarize()`, nous pouvons l'utiliser pour calculer des statistiques récapitulatives appropriées par sous-préfecture. Commençons par appeler un `summarize()` vide et regrouper les données sur `sub_prefecture`.
...
Nous voulons d'abord examiner le nombre de cas dans chaque `sub_prefecture`. Cela peut être fait en utilisant la fonction d'aide `n()` : 

```{r}
df_linelist |>
  summarize(.by = sub_prefecture,
            n_patients = n() # Comptages)
```
...
::: {.write}
Ajoutez la date d'admission minimale à votre tableau pour chacune des `sub_prefecture` ? Êtes-vous satisfait des résultats ? 
:::
...
Cette somme nous permet de compter toutes les lignes où notre condition a été remplie (retourne `TRUE`). Par exemple : ```{r} df_linelist |> summarize( .by = sub_prefecture, n_female = sum
...
Pour donner un exemple concret, si nous voulions calculer la moyenne de la variable `age` mais *uniquement* pour les patients hospitalisés (c'est-à-dire dans les lignes où `hospitalisation == "yes"`) nous écririons :
...
::: {.callout-tip}
Vous voulez compter les lignes (donc utiliser `sum()`) qui remplissent une condition spécifique pour le résultat (`outcome == "dead"`), mais *uniquement* lorsque `age_group == "< 6 months"`
:::
