...
::: {.look}
In the Windows Explorer, look at the course folder. You should now see a new file with the extension `.Rproj` that has a small blue icon with an R in it.
:::
...
In general, the usage of `import()` is pretty simple, at a minimum, you need to pass the path of the file to the `file` argument:
