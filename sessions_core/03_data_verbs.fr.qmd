...
Nous pouvons enfin travailler sur nos données ! Cette leçon et les suivantes s'appuieront sur plusieurs paquets de la *collection de paquets* [tidyverse]{.hovertip bs-toggle='tooltip' bs-title="Le tidyverse est une collection de paquets pour R qui insistent sur la lisibilité du code et la facilité d'utilisation."} pour manipuler des data frames, résumer et visualiser les données, et en particulier paquet `{dplyr}`.
...
:::{.callout-note}
Peut-être avez-vous noté que nous présentons le paquet `{dplyr}` mais nous vous avons fait charger le paquet `{tidyverse}` lors de la mise en place. Le `{tidyverse}` est un *méta-paquet*: le charger charge automatiquement [plusieurs des paquets les plus utiles]{.hovertip bs-toggle='tooltip' bs-title="L'univers du tidyverse contient de très nombreux paquets, et les charger tous d'un coup serait long et peu utile. Les paquets les plus courants  ont donc été rassemblés dans un méta paquet `{tidyverse}`, pour un chargement rapide"} de l'univers du tidyverse, dont fait partie `{dplyr}` et d'autres paquets que nous verrons dans la session.
:::
