...
Mise en place
-------------------------------------------------------------------------
**Prérequis** : cette leçon part du principe que vous savez comment utiliser RStudio et que vous êtes capable d'importer des données. Rafraîchissez-vous si besoin avec les [deux premières leçons](https://epicentre-msf.github.io/repicentre/pathway.html). Nous allons également poursuivre l'étude de cas sur la rougeole de Mandoul ; **si vous n'avez pas encore suivi les autres sessions du cours, veillez à [télécharger le dossier du cours](https://github.com/epicentre-msf/repicentre/raw/main/data/scenarios/TCH_measles/mandoul_measles.zip) avant de continuer**.
...
Quand nous n'enregistrons pas le data frame renvoyé par la fonction, il s'affiche dans la console, ce qui est peu lisible : il est laborieux de vérifier que votre commande a bien renommé les colonnes (ce serait encore pire si le data frame contenait 100 colonnes !). Une [première stratégie]{.hovertip bs-toggle='tooltip' bs-title="Nous en verrons une autre à la fin de la leçon avec le pipe, et une dernière est d'imbriquer la commande à tester directement dans une autre fonction, telle que names()."} consiste à enregistrer le data frame intermédiaire dans un objet pour pouvoir l'examiner avec des fonctions connue. Vous pouvez nommer cet objet temporaire comme vous voulez, mais un nom commun est `temp` (ou `tmp` en anglais) pour indiquer que ce n'est pas un objet final à être conservé.
