...
description: Une introduction au traitement et au nettoyage des données à l'aide du paquet `{dplyr}`.
...
df_brut <- rio::import(file = here::here('data', 'scenarios', 'TCH_measles',
...
Mise en place
-------------------------------------------------------------------------
**Prérequis** : savoir utiliser RStudio et importer des données. Rafraîchissez-vous si besoin avec les [deux premières leçons](https://epicentre-msf.github.io/repicentre/pathway.html).

Nous allons poursuivre l'étude de cas sur la rougeole à Mandoul; **si vous n'avez pas encore suivi les autres sessions du cours, veillez à [télécharger le dossier du cours](https://github.com/epicentre-msf/repicentre/raw/main/data/scenarios/TCH_measles/mandoul_rougeole.zip) avant de continuer**.

::: {.setup}
Nous utiliserons la linelist avec des données **brutes** que vous avez importée lors de la leçon précédente. 
<br><br>
Créez un nouveau script appelé `fonctions_donnees.R` dans votre sous-dossier `R`. Ajoutez un en-tête approprié et chargez les paquets suivants : `{here}`, `{rio}` et `{tidyverse}`. Puis, ajoutez une section dédiée à l'import des données, utilisez `{here}` et `{rio}` pour importer vos données dans R, et assignez-les à un objet que nous appellerons [`df_brut`]{.hovertip bs-toggle='tooltip' bs-title="Notez que nous ne nommons pas tout de suite l'objet df_linelist comme nous l'avions fait précédemment. La raison deviendra manifeste sous peu."}.
:::
...
Nous pouvons enfin travailler sur nos données ! Cette leçon et les suivantes s'appuient sur plusieurs paquets de la *collection de paquets* [tidyverse]{.hovertip bs-toggle='tooltip' bs-title="Le tidyverse est une collection de paquets R qui insistent sur la lisibilité du code et la facilité d'utilisation."} pour manipuler des data frames, résumer et visualiser les données, et en particulier le paquet `{dplyr}`.

Le *traitement des données* (parfois appelé *manipulation des données*) est un ensemble d'actions essentielles pour préparer et nettoyer les données avant une analyse (que ce soit dans R ou Excel). Le paquet `{dplyr}` fournit de nombreuses fonctions pour remplir ce genre de tâches courantes :
...
Ces fonctions ont en général un [nom intuitif, qui correspond à un verbe]{.hovertip bs-toggle='tooltip' bs-title="Au moins en anglais, mais nous vous fournirons la traduction. Comme les auteurs du paquet utilisent souvent des verbes pour nommer leurs fonctions, on parle souvent des 'verbes de dplyr'."}. Par exemple, pour renommer des colonnes, on utilisera la fonction `rename()`.

Aujourd'hui nous nous focaliserons sur les quatre verbes (fonctions !) qui permettent d'effectuer les tâches mentionnées précédemment, et que vous utiliserez en permanence. Nous apprendrons également à enchaîner les actions dans un *"pipeline"* pour plus de fluidité.


:::{.callout-note}
Nous vous avons fait charger le paquet `{tidyverse}`lors de la mise au lieu de `{dplyr}`. Le `{tidyverse}` est un *méta-paquet* : le charger charge automatiquement [plusieurs paquets très utiles]{.hovertip bs-toggle='tooltip' bs-title="L'univers du tidyverse contient de très nombreux paquets. Les charger tous d'un coup serait long et inutile. Les paquets les plus courants ont donc été rassemblés dans un méta paquet `{tidyverse}`, pour un chargement rapide"} de l'univers du tidyverse, dont fait partie `{dplyr}` et d'autres paquets que nous verrons dans la session.
:::
...
Si l'on veut supprimer des colonnes d'un data frame un peu [large]{.hovertip bs-toggle='tooltip' bs-title="Vous avez sans doute déjà été confronté à des listes linéaires ou des données d'enquête longues et complexes"} il suffit de préfacer les noms de colonnes par l'opérateur soustraction (`-`). 
...
Si vous ne faites qu'afficher le data frame produit dans la console (sans l'assigner à un objet), il peut être dur de voir si les colonnes ont été correctement renommées. Une [première stratégie]{.hovertip bs-toggle='tooltip' bs-title="Une seconde utilise le pipe, que nous verrons à la fin de cette leçon, et une troisième est d'imbriquer la commande à tester directement dans une autre fonction, telle que names()."} consiste à enregistrer le data frame intermédiaire dans un objet pour pouvoir l'examiner avec des fonctions connues. Vous pouvez nommer cet objet temporaire comme vous voulez, mais un nom commun est `temp` [ou `tmp` en anglais] pour indiquer que ce n'est pas un objet destiné à être conservé.


:::{.write}
Utilisez le code que vous venez d'écrire et créez un objet `temp` qui contient le data frame avec les collones renommées. Utilisez ensuite la fonction `names()` pour vérifier que les noms des colonnes ont bien changé. La sortie de `names()` devrait être :
...
Fantastique ! C'est un excellent début de pipeline de nettoyage de vos données. Sauvegardez ce code, car nous le compléterons lors de la prochaine session, durant laquelle nous apprendrons à recoder les variables, une autre étape essentielle du traitement de données !
