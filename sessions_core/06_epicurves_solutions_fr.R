...
# Description : Code pour créer une courbe épi du nombre de cas par jour de début des symptômes.
...
# Compte le nombre de cas par jour de date de début de symptômes
...
# Si vous n'utilisez le jeu de données agrégées qu'une seule fois, vous pouvez compter
# les cas directement dans le pipe avant de tracer le graphe
df_linelist %>%
  count(date_onset) %>%
  ggplot(aes(x = date_onset,
             y = n)) +
