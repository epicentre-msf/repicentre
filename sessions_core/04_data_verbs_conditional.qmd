...
:::{.callout-tip}
For the last step you can use the `%in%` operator that we saw today, or go read the help page of the [`str_detect()` function](https://stringr.tidyverse.org/reference/str_detect.html) and adapt the code to use in a `filter()`.
:::
...
You need the functions `count()`, `filter()` and `export()` + `here()` friend to do all that.
:::
