...
1. **Filter**: create data frames that return only:
...
2. **Cleaning**: add steps to the cleaning pipeline:
...
3. **Refactor**: the following code runs without errors but is longer than it needs to be because the author did not fully understand how `case_when()` works. Simplify the command. Do not forget to check that you obtain the same result at the end!
...
4. **Conditions**: you can use conditions outside of `filter()` and `case_when()`. Let's try thm with vectors: let us consider the two following vectors: 
```{r}
#| eval: true
vec1 <- c(1, 2, 3, 4, 5)
vec2 <- c(1, 3, 4, 2, 5)
...
5. Write a command to check if the facility "Moissala Ouest" exists in our linelist.
