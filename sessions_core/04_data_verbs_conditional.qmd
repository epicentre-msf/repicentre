...
- Are the values of `vec1` superior or equal to `vec2`? The output should look like `r vec1 >= vec2`.
- Are the values of `vec1` present anywhere in `vec2`? The output should look like `r vec1 %in% vec2`.
- Test is the values of `vec2` are `NA`. **Hint: you can use the `is.na()` function.

2. Write a command to check is the facility "Moissala Ouest" exists in our linelist.
...
- Patients from Moissala who where hospitalized in Moissala Est hospital before the fist of February 2023
- Severe acute malnourished children under the age of five years

4. Cleaning: add steps to the cleaning pipeline:

- Use title case for the `hospitalisation` and `malaria_rdt` column
- Recode the `outcome` column to standardize the values to "Dead", "Recovered", "Lost to follow-up"
- Create a `is_dead` column that is `TRUE` is the patient died, and `FALSE` if not
- Create a simplified `vacc_status_simple` column with modalities "Yes", "No", "Unknown"


5. The following code runs without errors but is too long because the writer did not take advantage of one of the rule of `case_when()`. Simplify the commend (check that you obtain the same result at the end!).
...
6. Someone tried to fix the above code but made mistakes. Can you fix them? You know you are done when you have the same counts as before.
