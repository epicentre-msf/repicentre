...
5. **Refactor**: the following code runs without errors but is too long because the writer ignored one of the rule of `case_when()`. Simplify the command. Do not forget to check that you obtain the same result at the end!
...
df |> 
  mutate(age_group2 = case_when(
    ...
    age_months >= 12 &  age_months < 59 ~ "12 - 59 months",
    age_years >= 5 & age_years <= 15 ~ "5 - 15 years",
    age_years > 15 ~ "> 15 years",
...
6. **Fix**: someone tried to fix the above code but made mistakes (the error does not cause the code to fail to run, but the output is wrong). Can you fix them? You know you are done when you have the same counts as before.
