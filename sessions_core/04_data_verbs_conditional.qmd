...
df_raw <- rio::import(file = here::here('data', 'scenarios', 'TCH_measles',
                                        'raw', 'moissala_linelist_EN.xlsx'))
```
...
We will also be continuing with the Mandoul measles case study; **if you haven't done the previous course sessions then be sure to [download the course folder](https://github.com/epicentre-msf/repicentre/raw/main/data/scenarios/TCH_measles/mandoul_measles.zip) before moving forward**.



::: {.setup}
This session will work with the **raw** Mandoul linelist data.
<br><br>
Create a new script called `filtering_and_recoding_practice.R` in your `R` directory. Add an appropriate header and load the following packages: `{here}`, `{rio}`, and `{tidyverse}`. Finally, add an import section where you use `{here}` and `{rio}` to load your data into an object called `df_raw`.
:::
...
:::{.write}
Create a filter that selects all patients with severe accute malnutrition (ie: patients with a MUAC less than 115). The head of `muac` should look like this:
...
So for example, if we want our `case_when()` to say that anytime a patient had a MUAC less than 115 we want to have a value of `"SAM"`, we would have something like this:
...
```{r}
df_raw |>
  mutate(malnut = case_when(muac < 115 ~ 'SAM',
                            muac < 125 ~ 'MAM',
                            .default = 'Normal'))
```
...
- Does this patient have SAM (is `muac < 115`)? **If so**, assign the value `"SAM"`
...
:::{.write}
Use `case_when()` and the `%in%` operator to create a new column `vacc_status_simple` that has the value `"Yes"` for cases with [**either** card or verbal confirmation]{.hovertip bs-toggle='tooltip' bs-title="Had a vacc_status of 'Yes - card' or 'Yes - oral'"}, `"No"` for cases who said they were [unvaccinated]{.hovertip bs-toggle='tooltip' bs-title="Had a vacc_status of 'No'"}, and `"Unknown"` otherwise. The head of your new column should look like this:
...
df_raw |>
  mutate(vacc_status_simple = case_when(vacc_status %in% c('Yes - card', 'Yes - oral') ~ 'Yes',
                                        vacc_status == 'No' ~ 'No',
                                        .default = 'Unknown')) |>
  select(vacc_status, vacc_status_simple) |>
  head()
```
:::
...
downloadthis::download_link(
  link = 'https://github.com/epicentre-msf/repicentre/blob/main/sessions_core/04_data_verbs_conditional_solutions.R',
...
1. **Conditions**: let us consider the two following vectors: 
```{r}
#| eval: true
vec1 <- c(1, 2, 3, 4, 5)
vec2 <- c(1, 3, 4, 2, 5)
...
-  Is `vec1` equal to `vec2`? The output should be `r vec1 == vec2`.
-  Are the values of `vec1` superior or equal to `vec2`? The output should be `r vec1 >= vec2`.
-  Are the values of `vec1` present anywhere in `vec2`? The output should be `r vec1 %in% vec2`.
-  Test if the values of `vec2` are `NA`. **Hint**: you should use the `is.na()` function.

2. Write a command to check if the facility "Moissala Ouest" exists in our linelist.

3. **Filter**: create data frames that returns only:

    - Patients from the Bedaya sub-prefecture
    - Patients with no fever
    - Patients who tested negative for malaria
    - Patients with no missing information in the fever column
    - Patients who were hospitalized in 2023
    - Patients of 15 years or more who were hospitalized at any time
    - Patients from Moissala who where hospitalized in Moissala Est hospital before the first of February 2023
    - Severe acute malnourished children under the age of five

4. **Cleaning**: add steps to the cleaning pipeline:

    - Use title case for the `hospitalisation` and `malaria_rdt` columns
    - Recode the `outcome` column to standardize the values to "Dead", "Recovered" and "Lost to follow-up"
    - Create a `is_dead` column that is `TRUE` if the patient died, and `FALSE` if not
    - Create a simplified `vacc_status_simple` column with modalities "Yes", "No", "Unknown"


5. **Refactor**: the following code runs without errors but is longer than it needs to be because the author did not fully understand how `case_when()` works. Simplify the command. Do not forget to check that you obtain the same result at the end!
...
df |> 
  mutate(age_group2 = case_when(
    ...
    age_months >= 12 &  age_months < 60 ~ "12 - 59 months",
...
6. **Fix**: someone tried to fix the above code but made mistakes (the code runs but the output is wrong). Can you fix them? You know you are done when you have the same counts as before.

```{r}
df |> 
  mutate(age_group2 = case_when(
    age_months <= 6 ~ '< 6 months',
    age_months < 11 ~ '6 - 11 months',
    age_years < 5 ~ '12 - 59 months',
    age_years <= 15 ~ '5 - 14 years',
    age_years > 16 ~ '15 + years',
    .default = 'Unknown')
  )
```
...
1. Save a `.csv` file on your computer that contains the clean data for patients from Bouna hospitalized in 2022.

2. Print a data frame containing the *number of patients* coming from the Moissala sub-prefecture that attended one of the Moissala facilities (here, we consider the facilities with "Moissala" in their name).

**Hint.** For the last step you can use the `%in%` operator that we saw today, or go read the help page of the [`str_detect()` function](https://stringr.tidyverse.org/reference/str_detect.html) and adapt the code to use in a `filter()`.
...
3. Create an Excel file that contains a list of IDs that are present more than once in the *clean data*. Do you understand why these IDs may need to be examined?

**Hint.** You need the functions `count()`, `filter()` and `export()` + `here()` friend to do all that.
