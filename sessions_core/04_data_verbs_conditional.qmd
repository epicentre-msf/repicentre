...
Write the following tests:
  ...
  - Test is the values of `vec2` are `NA`. **Hint: you can use the `is.na()` function**.

2. Write a command to check if the facility "Moissala Ouest" exists in our linelist.

3. **Filter**: create data frames that returns only (print them, no need to save them in objects):
...
5. **Refactor**: the following code runs without errors but is too long because the writer did not take advantage of one of the rule of `case_when()`. Simplify the command. Do not forget to check that you obtain the same result at the end!
...
6. **Fix**: someone tried to fix the above code but made mistakes. Can you fix them? You know you are done when you have the same counts as before.
...
**Tip** For the last step you can use the `%in%` operator that we saw today, or go read the help page of the [`str_detect()` function](https://stringr.tidyverse.org/reference/str_detect.html) and adapt the code to use in a `filter()`.
