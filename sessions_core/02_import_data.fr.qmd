...
Une fois qu'un paquet est installé, il faut indiquer à R que nous souhaiton l'utiliser pour une  session donnée. C'est ce qu'on appelle *le chargement du paquet*,  qui s'effectue avec la fonction `library()`.
...
::: {.write}
Utilisez la fonction `library()` pour charger les paquets `here` et `rio` qui seront utilisés dans la section suivante.
:::
