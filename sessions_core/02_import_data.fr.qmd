...
- Cliquez sur [le bouton]{.hovertip bs-toggle='tooltip' bs-title="Vous pouvez cliquer sur ce bouton à tout moment pour savoir dans quel projet vous travaillez actuellement."} `Project: (none)` en haut à droite de l'interface RStudio
...
::: {.write}
 Utilisez [des commentaires]{.hovertip bs-toggle='tooltip' bs-title="Vous pouvez utiliser les raccourcis clavier CTRL+SHIFT+C and CTRL+SHIFT+R pour insérer des commentaires ou des sections"} pour créer une section "Paquets" dans votre script.
:::
...
Nous pouvons également vérifier nos données en regardant l'onglet *Environment* dans le panneau supérieur droit. Ici, nous pouvons voir notre dataframe dans l'environnement, examiner sa [structure]{.hovertip bs-toggle='tooltip' bs-title="Cela reivent á utiliser la commande `str()`"} ou l'ouvrir dans le visualiseur de données ("data viewer") de RStudio.
...
Le visualiseur de données affiche les dataframe sous forme de tableaux et constitue un moyen pratique d'examiner rapidement vos données. Vous pouvez même trier et filtrer vos données dans "View", mais sachez que ces actions n'apporteront aucune modification à l'objet réel. `df_linelist`."View" peut également être ouvert en utilisant directement la fonction `View()`.
