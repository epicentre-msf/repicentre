...
10. Train with pipe: rewrite the following commands, which we have sees today or in past sessions using `{dplyr}` verbs and the *pipe*
...
The code below contains steps to create a `df1` data frame written in base R syntax. It uses function and syntax that we have seen in all the previous sessions. Can you rewrite it with `{dplyr}` verbs and the *pipe* to obtain an identical `df1` data frame?
