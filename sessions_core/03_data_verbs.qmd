...
10. Train with pipe: rewrite the following commands, which we saw today or in past sessions using `{dplyr}` verbs and the *pipe*
...
The code below contains steps to create a `df1` data frame written in base R syntax. It uses functions and syntax that we have seen in all the previous sessions. Can you rewrite it with `{dplyr}` verbs and the *pipe* to obtain an identical `df1` data frame?
...
::: {.callout-tip}
To debug a pipeline, run it step by step (by pairs of statement `input |> function()`) to identify problems that may arise at each step (inspect the error if error there is, and the intermediate data frame created). Make your way down the pipe to fix each step that needs to be fixed.
:::
