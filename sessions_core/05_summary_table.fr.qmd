...
**Prérequis** : cette session part du principe que vous avez effectué les sessions obligatoires précédents, en particulier que vous savez importer vos données et utiliser les verbes principaux de `{dplyr}`. Aller vous [rafraîchir si besoin](03_data_verbs.qmd).
...
Nous pouvons faire ceci avec la fonction `filter()`. Nous avons vu dans la [session précédente](../sessions_core_04data_verbs_conditional.qmd) que l'on peut tester l'inégaliter avec l'opérateur `!=`. par contre, comme souvent avec R, le travail avec les valeurs `NA` est un peu particulier. Pour tester si une valeur est `NA` on utilise la fonction `is.na()`. Pour tester si une valeur n'est *pas* `NA`, on préface la fonction d'un signe `!`, qui est indique à R de "faire le contraire": `!is.na()` teste si une valeur n'est *pas* `NA` !
...
:::{.write}
Reprenez votre code précédent et ajouter un filtre pour calculer les proportions seulement sur les patients guéris et décédés. Calculez les effectifs comme précédement et les proportions. La proportion des décès est la létalité. Stockez ce nouveau data frame dans un objet `df_sortie` qui resemble à ceci : 
```{r}
#| eval: true
#| echo: false
...
Maintenant que nous avons produit quelques tableaux de fréquence simples, nous pouvons augmenter la complexité. Une tâche courante en épidémiologie consiste à examiner les statistiques résumées *par groupe* d'une variable (ie stratifiés par une variable catégorique). 

Dans ce tutoriel, nous allons ensemble construire une grosse table rassemblant de nombreuses statistiques sur les patients stratifiées par sous-préfecture. Nous vous montrerons comment créer les statistiques descriptives suivantes :
...
Notez que ce code renvoit *une seule valeur* pour l'âge moyen. Aucune variable n'a été fournie par laquelle stratifier, donc `summarize()` a renvoyé une statistique récapitulative pour l'ensemble du data frame. Pour calculer l'âge moyen *par groupe* (ie, stratifié par une variable), nous devons fournir une variable (ou plusieurs dans un vecteur) à l'argument `.by` :
...
::: {.write}
Ajouter la ligne pour calculer l'âge moyen par sous préfecture dans votre table. Puis ajoutez une colonne `min_admission` qui contient la date d'admission minimale par sous-préfecture à votre tableau. Êtes-vous satisfait des résultats ? 
:::
...
::: {.write}
Ajoutez la ligne pour compter le nombre de femmes, puis ajoutez une variable `n_hosp` à votre tableau qui compte le nombre de patients hospitalisés (c'est-à-dire les lignes qui ont "oui" dans la variable `hospitalisation`).
...
::: {.write}
Ajoutez le calcul de l'age moyen des patients hopsitalisés à votre table, puis ajoutez une colonne `moy_age_femmes` pour calculer l'âge moyen des patientES (des femmes).
