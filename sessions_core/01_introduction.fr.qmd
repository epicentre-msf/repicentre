...
::: {.write}
Mettez à jour `cases` avec les valeurs ci-dessus et mettez à jour `region` pour créer un vecteur de chaînes de caractères contenant les valeurs suivantes : Mandoul, Moyen-Chari, Logone Oriental, Tibesti et Logone Occidental.
:::
...
```{r}
#| eval: true
mean(cases)      # Calcule la moyenne des valeurs stockées dans le vecteur
...
- calculer la somme des valeurs de `cases` avec la fonction `sum()`
...
```{r}
#| eval: true
cases[2]   # Deuxième valeur de cases
cases[10]  # Dixième valeur de cases
```

Oups il n'y a pas de dixième valeur dans `cases` ! Nous reviendrons sur ce que ce `NA` signifie dans la section [valeurs manquantes](../sessions_core/01_introduction.qmd#sec-missing-values).
...
```{r}
#| eval: true
cases[2:4]  # de la deuxième à la quatrième valeur
```
...
::: {.write}
Dans votre script, créez un dataframe nomé `data_cases` qui contient `cases` dans une colonne et `region` dans l'autre.
:::
...
#### Exploration d'un dataframe {#sec-dataframe-functions}

L'objet `data_cases` devrait maintenant apparaître dans votre environnement. Vous pouvez cliquer sur le cercle bleu avec un triangle blanc pour dérouler des informations supplémentaires, ou cliquer sur son nom pour le visualiser dans un onglet dans le même volet que votre script.

![dataframe `data_cases` apparaît désormais dans l'onglet Environnement.](../img/core/01_introduction/03_environment.png){fig-align="center"}

Il existe plusieurs fonctions pratiques pour explorer un dataframe :

::: {.write}
Exécutez les commandes suivantes et essayez de déterminer le type d'informations qu'elles renvoient.

```{r}
str(data_cases)     # STRucture de l'object
dim(data_cases)     # DIMension de l'object
nrow(data_cases)    # Nombre de lignes (row = ligne)
ncol(data_cases)    # Nombre de COLonnes
names(data_cases)   # noms des colonnes
```

:::
Pratiquons un peu plus ! R est livré avec quelques dataframes intégrés, auxquels il est possible d'accéder directement, dont un appelé `iris`. C'est pratique pour cette session car nous n'avons pas encore appris à importer des données dans R (ne vous inquiétez pas, nous travaillerons sur des données de liste linéaire dès la prochaine session !).

Nous pouvons afficher les premières lignes de ce dataframe grâce à la fonction `head()` (head = la tête en anglais):
...
::: {.write}
Combien de lignes et de colonnes y a-t-il dans `iris`? Quels sont les noms des colonnes de ce dataframe ?
:::

#### Accéder aux données d'un dataframe

En R, il existe plusieurs méthodes pour accéder aux lignes et/ou colonnes d'un dataframe. Dans cette session d'introduction, nous nous concentrerons sur la syntaxe `[row, column]`.
...
```{r}
#| eval: true
data_cases[1, 2]          # Afficher la valeur de la ligne 1, deuxième colonne
data_cases[1, "region"]   # Afficher la valeur de la lignbe 1, pour la colonne région
```
...
```{r}
#| eval: true
data_cases[1, ]           # Extraire la première ligne (garde toutes les colonnes)
data_cases[2:4, ]         # Valeurs des lignes 2 à 4, pour toutes les colonnes
data_cases[ , "region"]   # Garde toutes les lignes mais que la colonne région
```
...
```{r}
#| eval: true
data_cases[c(1, 3), ]  # Ligne 1 et 3 (toutes les colonnes)
```

Soyez attentifs, le *type* de sortie renvoyé lors de l'extraction des parties d'un dataframe dépend de l'indexation utilisée :

```{r}
#| eval: true
str(data_cases[1 , ])   # Renvoit un dataframe
str(data_cases[ , 1])   # Renvoit un vecteur
```

Une syntaxe simplifiée existe pour extraire des colonnes d'un dataframe :

```{r}
#| eval: true
data_cases[2]           # Renvoit la deuxième colonne (format dataframe)
data_cases["region"]    # Renvoit la colonne région (format dataframe)
```

Notez que ces commandes renvoient des dataframes avec une seule colonne.

::: {.write}
Ecrivez le code pour :

- extraire la troisième valeur de la colonne `region` de votre dataframe
- extraire les deuxième et troisième valeurs de la colonne `cases`
- calculer la somme des valeurs de la colonne `cases`
:::
...
```{r}
mean(cases,na.rm=TRUE)     # MAUVAIS
mean(cases, na.rm = TRUE)  # BON
```
...
```{r}
#| error=TRUE
mean(TRUE, cases)  # Pas ce que vous attendez
```
