...
Au cours de ces exercices, vous rencontrerez certainement des **erreurs**, qui se produisent lorsque R n'est pas en mesure d'exécuter une commande. Les raisons sont multiples : une faute d'orthographe dans le nom d'un objet ou d'une fonction, le mauvais type de données fournis etc. Lorsqu'une erreur se produit, R **arrête** les calculs en cours et affiche un message expliquant ce qu'il s'est passé. **Les erreurs arrivent à tous les programmeurs, novices comme experts**. Comme quand on apprend une langue, l'amélioration vient avec la pratique, en faisant des erreurs et en apprenant à les corriger.
...
::: {.setup}
Créez un script et enregistrez-le sous le nom `decouverte.R` (n'oubliez pas l'extension !). Pour l'instant, vous pouvez l'enregistrer sur votre bureau ou à tout autre endroit pratique, mais nous aborderons l'organisation des scripts dans la [prochaine session](../sessions_core/02_import_data.qmd#folder-structure).
:::
...
# Description : Importat et nettoyage des données de surveillance rougeole de Mandoul
```
...
Le type numérique englobe les *entiers* [*integers* en anglais] et les *nombres décimaux* [*doubles*]. Les nombres en R n'ont pas de signalétique, tapez simplement la valeur brute dans votre script ou votre console.
