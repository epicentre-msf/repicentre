...
data.frame(col1 = c(1, 4, 2, 9),
           col2 = c("un peu de texte", "plus de texte", "Salut !", "les epidemiologistes !"))
```
...
::: {.write}
Dans votre script, créez un data frame nommé `data_cas` qui contient `cas` dans une colonne et `region` dans l'autre.
:::
...
data_cas[1, "region"]   # Afficher la valeur de la ligne 1, pour la colonne région
```
