...
En tant qu'épidémiologistes, nous sommes constamment confrontés aux données manquantes. Dans R, celles-ci sont codées à l'aide d'une valeur spéciale : `NA` (signifiant **N**ot **A**vailable). La valeur `NA` est spéciale en R, car elle n'a pas de *type* fixe, elle prend le *type* des valeurs qui l'entourent. Par exemple, un `NA` dans une colonne numérique prend le type numérique. Nous aurons des occasions de manipuler les `NA` dans la suite du cours.
