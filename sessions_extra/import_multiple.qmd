---
title: Import Multiple Files/Sheets
...
This satellite builds on the [Import data core session](../sessions_core/02_import_data.qmd), so we expect you to be working in the course project. You can add the commands from this tutorial to the `import_data.R` script that you created in that session, or create a new script. 


::: {.setup}
Download and decompress the below folder containing data and an `.Rproj` file for this session. Navigate into it nd Place it inside a folder R  the below zip folder and save it in the `data/raw` subfolder of your course project. If you do not have a course project, either go to the [Import data core session](../sessions_core/02_import_data.qmd) to download it or create a RStudio project in a folder, and make sure you have a `data/raw` subfolder to store the data.
...
- You have an Excel file with data in several sheets, but this time [all the sheets are formatted the same way]{.hovertip bs-toggle='tooltip' bs-title="You probably encountered linelist or aggregated data where one sheet represents one week, or one district, and there are about 20 sheets..."}, and you wish to gather the data in the same dataset.
...
The `import_list()` function can import all the sheets from an [Excel file]{.hovertip bs-toggle='tooltip' bs-title="Or, in fact, all items from a multi-object file such as .Rdata or .zip"} and store them into a list. It avoids you having to write multiple `import()` commands for each file separately.
...
Now imagine someone gave you lab data with the positive, negative and inconclusive scattered in [three different sheets]{.hovertip bs-toggle='tooltip' bs-title="You wonder why they did that but you probably have seen weirder things."}. Let's import this data.
...
**Hint:** There are buttons that you can click on both on the left and right of the Viewer pane, but the ones on the right appear only when you hover your mouse over a line.
:::

You can access elements of the list with [these commands]{.hovertip bs-toggle='tooltip' bs-title="We added the head() function to shorten the output, but you can remove it to print the whole elements."}
...
# Access by position the first element of the list:
lab_data_list[1]
```
...
::: {.look}
Go to the sub-folder `data / raw /satellite_multiple/multiple_files/EN`. Open the Excel files to check their content. Would it make sense to concatenate the data together at import in this case?
:::
...
The `pattern` argument accepts regular expressions, or regex for short, which are [*way* out of our scope here]{.hovertip bs-toggle='tooltip' bs-title="Regular expressions have their own syntax that is not limited to R. They are super powerful but also hard to use so if you are new to R you should probably wait a bit before diving into them."}. Here we are only giving you pointers to create a simple _pattern_.
...
- the `\\.` at the start is the [regex way]{.hovertip bs-toggle='tooltip' bs-title='It sounds complicated because the "." alone has a special meaning in regex, so we need to add the \\\. Trust us for now.'} of saying that there should be a "." before "doc".
...
downloadthis::download_link(
  link = 'https://github.com/epicentre-msf/repicentre/blob/main/solutions/extra/import_multiple.R',
...
An introduction to [regular expressions](https://r4ds.hadley.nz/regexps)
