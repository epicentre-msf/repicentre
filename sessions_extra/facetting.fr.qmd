---
title: Introduction aux graphiques multiples (facetting)
...
**Prérequis** : ce satellite s'appuie sur la session sur les [courbes épidémiques](../session_core/06_epicurves.qmd), durant laquelle nous avons appris à visualiser la distribution quotidienne des cas de rougeoles au cours du temps à l'aide de `{ggplot2}` :
...
Si le graphique est très large, il est possible que les étiquettes des dates ne soient pas très lisibles en x, et c'est le cas pour certains des exemples. Cela peut être contrôlé, et le sujet est abordé dans un [autre satellite](../session_extra/weekly_epicurves.qmd) !
...
downloadthis::download_link(
  link = 'https://github.com/epicentre-msf/repicentre/blob/main/solutions/extras/facetting_solutions.fr.R',
...
::: {.callout-important}
Être capable de lire du code écrit par d'autres, de comprendre ce qu'il fait et de l'adapter à vos besoins  sont des compétences **très importantes** à cultiver. Cela reste vrai que le code provienne des pages d'aide officielles, de StakOverflow, de tutoriels ou de sorties de Large Language Models (comme Github Copilot, ou ChatGPT).
:::
 
Alors, un petit défi ?!

Les pages d'aide des paquets `{tidyverse}` sont généralement bien écrites. Nous vous conseillons de les lire lorsque vous apprenez une nouvelle fonction, vous y découvrirez plein d'astuces !

Pour vous faire travailler tout cela... Consultez les exemples de la [page d'aide](https://ggplot2.tidyverse.org/reference/facet_wrap.html) de `facet_wrap()` : l'un d'entre eux montre comment créer ce type de figure :
...
ggplot(data = df_age,     # general dataset
  ...
  # Plot the gray bars:
  geom_col(data = df_cas,     # dataset for just this layer
           fill = "grey80") +
...
Arrivez-vous à modifier votre code pour faire la même chose ?


::: {.callout-tip collapse="true"}
Il vous faudra deux data frame agrégés, l'un deux crée aujourd'hui et l'autre lors de la session sur les [courbes épis](../session_core/06_epicurves.qmd).
:::
