...
Since we are constrained in time, we are going to provinceide the code of the `rollapply()` function to calculate the cumulative sum over three weeks, but check out the details in the [Going further section](#sec-going-further) when you have time. 
...
In our case we want the value for a given week to reflect this week and the week past, so we align the window right.
:::
