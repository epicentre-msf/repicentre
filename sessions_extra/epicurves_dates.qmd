...
df_linelist <- rio::import(here::here("data", "clean",
  ...
  mutate(
   ...
   filter(date_consultation < as.Date("2023-01-01"))
...
In this tutorial we will learn to **display dates** on the x-axis of your graphs, and **tweak the date labels**.
...
In that graph we chose to represent epiweeks in the bottom axis, as it is a common task. In this session we will show you how to code a variant of this graph with dates instead.

We will plot the epicurve for outcomes in the *examples* and *you* will code the epicurve for consultations in the exercises.


## Setup

We will build on code from the [epicurve session](../sessions_core/07_epicurves.qmd) so you may code in the script associated with that session or create a new script.
...
It might be easier to understand if we calculate the day of thee week associated with each date using the function `wday()` (which *also* belong to the `{lubridate}` package, are you maybe seeing a pattern here 😉):  
...
This illustrates how `week_outcome` and `week_first_day_outcome` are two ways to have only one value by week, which makes aggregation possible at the week scale. One difference between the two options is that week numbers are not unique (2022 and 2023 have a first week) while dates are.
...
Notice that the bars are identical to the epicurve with weeks numbers. This is normal, as we are aggregating by week in both cases. It is only the information used to represent the week (ISO number or start date) that changes.

::: {.callout-note}
Actually, because the dates are unique, you could use the full dataset (with 2022 and 2023 consultations) for these epicurves!
:::
