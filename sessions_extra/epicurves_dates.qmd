...
description: Plot weekly epicurves and improve date labels on the axis
date: last-modified
...
df_linelist <- rio::import(here::here("data", "clean",
  ...
  mutate(
    ...
    # Week number (ISO)
    week_onset = isoweek(date_onset),         # Their code
    week_outcome   = isoweek(date_outcome),   # Examples
    
    # First DATE of the week
    week_first_day_onsetet = floor_date(date_onset,       # Their code
                                    unit = "week",
                                    week_start = 1),
...
This graph is aggregated by day, which can be a reasonable level of aggregation if the outbreak is short or if you wish to zoom on a short period. As epidemiologists we however often want to plot data **by week**.

In this tutorial we will learn two ways of aggregating data by week, plot the data and **tweak the date labels**.

We will plot the epicurve for outcomes in the *examples* and *you* will code the classic epicurve for date of onset in the exercises.
...
::: {.callout-important}
To avoid the warnings all along the session, we removed the missing values from the outcome date columns in the examples. When you analyse and plot data, always be aware of how many missing data you have and how a `tidyr::drop_na()` command affects your dataframe.
:::

## Aggregate Data by Week

There we are going to discuss two ways of aggregating data by weeks. You may be more familiar with the first one (using week _numbers_ to identify weeks), but we are going to focus more on a more robust way (using the _firs day of the week_ to identify weeks).



### Using Week Numbers

Probably the most intuitive way of thinking of weekly aggregated data is to think in terms of week numbers, as aggregated data from MoH are often in this format, and you probably created a lot of epicurves with week numbers.

The`isoweek()` from the `{lubridate}` take a date (or a vector of dates) and returns the associated [ISO week]{.hovertip bs-toggle='tooltip' bs-title="ISO weeks offer an internationally standardized approach to dates that avoid differences between countries on when a week starts and how to define the first week of the year. ISO weeks begin on Monday. If you are working with the US CDC, you may use the sister function epiweek() that starts on Sunday."}

```{r}
#| eval: true
example_date <- '24-02-2025'
...
We can use this function to create a `week_outcome` in our data frame:
...
The head of the `date_outcome` and `week_outcome` columns looks like this:

```{r}
df_linelist %>% 
  tidyr::drop_na(date_outcome) %>% 
  select(date_outcome, week_outcome) %>% 
  head()
```


::: {.write}
Your turn. After importing data, use the `mutate()` and `isoweek()` functions to create a new column in your dataframe called `week_onset` that contains the ISO week associated with every *onset date*. The head of `date_onset` and `week_onset` columns should look like this:
...
df_linelist %>% 
  ...
  count(year_onset, week_onset)
```

These counts are perfectly correct. You could plot them using [faceting](../session_extra/faceting.qmd) by year, or just filter a given year and plot the weekly numbers with the ISO week number on the x-axis. For example:
...
df_linelist %>% 
  ...
  geom_col(fill = "#2E4573") +
  theme_classic() +
  facet_wrap(vars(year_onset),
             ncol = 1)
```

If you have not read about faceting yet, do no worry, think of this as a teaser of how easily you can make subplots by a variable!

But this is out of the scope of this tutorial. Instead, we will show you another way of aggregating data by week which is robust to multi-year data in the next section.
...
It might be easier to visualise if we calculate the day of the week associated with each date using the function `wday()` (which *also* belong to the `{lubridate}` package, are you maybe seeing a pattern here 😉):  
...
df_linelist %>% 
  ...
  mutate(
    day_outcome = wday(date_outcome, 
                       label = TRUE, 
                       abbr = FALSE),
    day_first_day   = wday(week_first_day_outcome, 
                       label = TRUE, 
                       abbr = FALSE)) %>% 
  arrange(date_outcome) %>%     # Sort the data by date
  select(date_outcome,
...
This illustrates how `week_outcome` and `week_first_day_outcome` are two ways to have only one value representing a week. While week numbers are not unique as discussed before, dates are!

::: {.write}
Add a new command to your `mutate()` call and create the variable `week_first_day_onsetet` that contains the first day of the week for patient date of oncet. Choose your argument as if the first day of the week is a Monday.
:::
...
::: {.write}
Count the number of patients per week of of onset, using the week start (`week_first_day_onsetet`).
...
df_linelist %>% 
  ...
  scale_x_date(date_breaks = "2 months") +  # Define breaks
  theme_classic()
```
