...
Notice how the number of elements in the outputs decreases when the size of the window `k = ` increases.

![Moving averages with different windows sizes](../img/extra/smoothing/smooth-001-moving-average-1.png){fig-align="center"}
...
![Moving averages with different windows sizes and fill = NA](../img/extra/smoothing/smooth-001-moving-average-2.png){fig-align="center"}

To update the previous graph, we need to add one column to the data frame that `ggplot()` takes as an input with the data to draw the line; then we need to update the graph command to draw the line on top of the bars.
...
You may create an intermediate data frame with this data to pipe into the `ggplot()` function or not, as you prefer.
:::
...
data %>% 
  ggplot() +
  geom_col(aes(x = week_consultation,
               y = n)) +
  geom_line(aes(x = week_consultation,
                y = smoothed)) +
  ...
```
