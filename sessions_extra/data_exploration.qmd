...
::: {.setup}
For this session we will work with our **raw** Mandoul measles linelist.
<br><br>
Open a new script called `data-exploration.R`, and make sure packages `{here}`, `{rio}` and `{dplyr}` are loaded. Then, import the data into R as an object called `df_linelist`. 
:::
...
Finally, let's look at the values in our categorical variables. For this we can use frequency tables. This is handy as it allows us to quickly see:

1. The unique values in a categorical variable
2. The number of observations for each of those categories

This is done using the function `count()` from the [package `{dplyr}`]{.hovertip bs-toggle='tooltip' bs-title="We will learn more about this package in the next session."}, which accepts the a dataframe and the name of one or more columns as arguments. It will then count the **number of observations** of each unique element in that column. For example, let's see the possible values of the variable `sex`: 
