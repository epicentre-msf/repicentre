...
This extra session assumes that you have seen [introduced to R and R studio](01_introduction.html), and [data importation](02_import_data.html). Please go over these sessions first if that is not the case. 
:::
...
Right after importing some data into R, it is good practice to have a look at them. When talking of *data exploration* we usually try to answer a few questions about the data: 
...
- Observe the possible values in each categorical variables. 
...
1. Look at your *environment pane* in R studio and check for your data - the number next to it (`5230x25`) tells us it is a dataframe with `5230` rows and `25` columns.
...
These methods will only print the first 40 rows of your data because that's the limit of your console. Alternatively, you can use `View()` to see your data in a tabular form. This will open a new window with your data displayed like in an excel spreadsheet - **Importantly**, you can only display the data with this command and not modify any of them. 

:::{.callout-tip}
Be very careful with `View()` on large dataset as this may crash your R studio session - to avoid this, you can print the output in the console. 
:::
