...
# For the extra exercice at the end
...
This satellite builds on the core [epicurve session](../session_core/06_ggplot.qmd), which is a prerequiste. In that session, we learned how to create an epicurve of measles cases across time: 
...
Add a `week_admission` variable to that data frame that contains the ISO week of the date of admission, as we did in the  [epicurve session](../session_core/06_ggplot.qmd).
:::
...
You will need two aggregated data frames, one created today and one created in the [epicurve session](../session_core/06_ggplot.qmd).
:::
