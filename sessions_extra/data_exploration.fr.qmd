...
**Dependances.** Cette session supplémentaire suppose que vous avez suivi les sessions [introduction à R et R studio](01_introduction.html), et [importation de données](02_import_data.html).  
:::
...
downloadthis::download_link(
  link = 'https://github.com/epicentre-msf/repicentre/data/raw/moissala_linelist_FR.xlsx',
...
- Déterminer la classe des variables
...
1. Regardez votre *volet environnement* dans RStudio et vérifiez vos données - le nombre à côté (`5230x25`) nous indique qu'il s'agit d'un dataframe avec `5230` lignes et `25` colonnes.
2. Utilisez `dim()` sur vos données pour retourner un vecteur avec le nombre de lignes et le nombre de colonnes.
...
::: {.write}
En utilisant la méthode de votre choix, obtenez les dimensions de votre dataframe `df_linelist`.
:::
...
Il est également intéressant d'inspecter vos données, cela peut vous permettre de repérer plus facilement certaines incohérences, des variables avec beaucoup de valeurs manquantes, et cela vous permettra de voir à quelles valeurs s'attendre pour chacune d'entre elles. Vous pouvez ["print"]{.hovertip bs-toggle='tooltip' bs-title="Print est un mot de programmation qui signifie afficher une sortie dans votre console."} vos données dans la console par : 

1. En exécutant l'objet `df_linelist` seul (attention, vous ne voudrez peut-être pas faire cela si vous avez un grand ensemble de données).
2. Utilisez la fonction `head()` pour voir les 6 premières lignes (vous pouvez augmenter ce nombre en utilisant l'argument `n`)
2. Utilisez la fonction `tail()` pour voir les 6 dernières lignes (encore une fois, vous pouvez augmenter ce nombre en utilisant l'argument `n`)
...
Nous voulons maintenant vérifier la classe des différentes variables. C'est important car une partie du nettoyage des données consiste à s'assurer que les variables numériques sont de classe `numeric`, dates `Date`, et que les variables catégorielles sont de classe `factor` ou `character`. Vous avez déjà vu la fonction `class()`, qui permet de vérifier la classe d'un vecteur. Dans R, chaque variable d'un dataframe est un vecteur. Nous pouvons extraire toutes les valeurs de ce vecteur en utilisant le [`$` sign]{.hovertip bs-toggle='tooltip' bs-title="C'est ce qu'on appelle 'base indexing' dans R"}, et les passer à la fonction class : 
...
:::{.write}
Essayez d'extraire toutes les valeurs de la variable `sex`. Quelle est la classe de cette variable ? 
:::

Vous pouvez également utiliser `str()` sur votre dataframe pour vérifier la classe de toutes les variables à la fois : 
...
:::{.write}
Utilisez `str()` pour vérifier le type de données de chaque colonne. Y a-t-il quelque chose d'étrange ? Rappelez-vous que vous pouvez aussi utiliser des fonctions comme `is.character()` et `is.numeric()` si vous voulez tester le type d'une colonne particulière.
:::

Exploration des variables continues
----------------------------------------------------------------------------------------------------

Maintenant que vous savez comment extraire les valeurs d'une variable, vous pouvez vouloir explorer certaines de ces valeurs des variables numériques pour vérifier les incohérences. Cherchons des statistiques sommaires pour ces variables, et Base R fournit de nombreuses fonctions pratiques :
...
| moyenne() | Moyenne arithmétique | moyenne(x) | Valeur moyenne |
| médiane() | Valeur moyenne | médiane(x) | Valeur moyenne | médiane(x) |
| range() | Min et max | range(x) | Vecteur de (min, max) |
| IQR(x) | Q3 - Q1 | IQR(x) | Q3 - Q1 | IQR(x) | Q3 - Q1 | IQR(x) | IQR(x) | Q3 - Q1 |
| quantile() | Quantiles spécifiés | quantile(x, probs = c(0.25, 0.75)) | Quantiles demandés |
| sd() | Ecart-type | sd(x) | Ecart-type | var() | Variance |
| var() | Variance | var(x) | Variance | var(x) | Variance |
| var() | Variance | var(x) | Variance | sum() | Somme des valeurs | sum(x) | Somme |
